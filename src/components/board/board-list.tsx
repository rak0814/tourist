...
function PostStats({ postId, initialViews, initialComments }: { postId: string; initialViews: number; initialComments: number }) {
  ...
  useEffect(() => {
    ...
    supabase.from("post_likes").select("*", { count: "exact", head: true }).eq("post_id", postId).then(({ count }) => {
      setLikes(count ?? 0);
    });
    supabase.from("comments").select("*", { count: "exact", head: true }).eq("post_id", postId).then(({ count }) => {
      setComments(count ?? 0);
    });
  }, [postId]);
        ...
        <li key={post.id}>
          ...
          >
            ...
            <div className="flex items-center gap-2 text-xs text-zinc-400">
              ...
              <span>{post.date}</span>
              <span>·</span>
              <PostStats postId={post.id} initialViews={post.views} initialComments={post.commentCount} />
            </div>
          </Link>
        </li>
      ))}
